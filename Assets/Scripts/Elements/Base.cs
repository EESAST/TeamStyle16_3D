...
public class Base : Building
{
	...
	protected override IEnumerator AimAtPosition(Vector3 targetPosition)
	{
		...
		while (Data.GamePaused || Quaternion.Angle(head.rotation = Quaternion.RotateTowards(head.rotation, targetRotation, Settings.SteeringRate.BaseHead * Time.deltaTime), targetRotation) > Settings.AngularTolerance)
			yield return null;
		...
		while (Data.GamePaused || Quaternion.Angle(bigGuns.rotation = Quaternion.RotateTowards(bigGuns.rotation, targetRotation_BG, Settings.SteeringRate.BaseBigGuns * Time.deltaTime), targetRotation_BG) > Settings.AngularTolerance || Quaternion.Angle(smallGuns.rotation = Quaternion.RotateTowards(smallGuns.rotation, targetRotation_SG, Settings.SteeringRate.BaseSmallGuns * Time.deltaTime), targetRotation_SG) > Settings.AngularTolerance)
			yield return null;
	}
