...
{
	private const float ACTION_INDENT_LEVEL_1 = 10;
	private const float ACTION_NODE_MARGIN = 5;
	private const float ACTION_NODE_SPACING = 60;
	private const int BASE_LOOP_LIST_POSITION_OFFSET = 80;
	private const float ENUM_SELECTOR_WIDTH = 300;
	private const float ENUM_SELECTOR_WIDTH_MEDIUM = 120;
	private const float ENUM_SELECTOR_WIDTH_SMALL = 70;
	private const float HEADER_HEIGHT = 30;
	private const float LINE_HEIGHT = 20;
	private const int LOOP_LINE_OFFSET = 35;
	private const int LOOP_LIST_WIDTH = 360; //325;
	private const float PROGRESSION_HEADER_LABEL_WIDTH = 150;
	private const float state_overview_x = 5;
	private const float TEXT_AREA_LINE_HEIGHT = 14;
	private const float TOOLBAR_LETTER_HEIGHT = 30;
	private const float TOOLBAR_LETTER_WIDTH = 40;
	private const float VECTOR_3_WIDTH = 300;
	private const float WINDOW_BORDER_BOTTOM = 10;
	// Gui layout variables
	private const float WINDOW_BORDER_LEFT = 10;
	private const float WINDOW_BORDER_RIGHT = 10;
	private const float WINDOW_BORDER_TOP = 10;
	public static Texture2D lineTex;
	private static readonly Color LOOP_COLOUR = Color.blue;
	private static readonly Color LOOP_REVERSE_COLOUR = Color.red;
	private static GUIStyle m_header_gui_style;
	private static TextEffectsManager m_instance;
	private readonly int main_action_editor_x = 100;
	private int ANIMATION_IDX;
	private Vector2 CUSTOM_LETTERS_LIST_POS = Vector2.zero;
	private int edited_action_idx = -1;
	private bool editing_start_state = true;
	private EffectManager font_manager;
	private bool ignore_gui_change;
	private Vector2 LOOP_SCROLL_POS = Vector2.zero;
	private int loop_tree_width = 60;
	private bool m_display_loops_tree;
	private int m_mouse_down_node_idx = -1;
	private Vector2 m_mouse_down_pos;
	private Vector2 m_mouse_drag_pos;
	private float m_old_time;
	private bool m_paused;
	private bool m_previewing_anim; // Denotes whether the animation is currently being previewed in the editor
	private Rect[] m_state_node_rects;
	private float m_time_delta;
	// Gui Interaction variables
	private Vector2 MAIN_SCROLL_POS = Vector2.zero;
	// New Loop Vars
	private int new_loop_from;
	private int new_loop_to;
	private bool noticed_gui_change;
	private int state_overview_width;
	...
	private static TextEffectsManager Instance
	{
		get
		{
			if (m_instance == null)
				m_instance = (TextEffectsManager)GetWindow(typeof(TextEffectsManager));

			return m_instance;
		}
	}

	private int MainEditorX { get { return state_overview_width < main_action_editor_x ? main_action_editor_x : state_overview_width; } }

	private bool CheckGUIChange() { return CheckGUIChange(font_manager.EditorActionIdx, font_manager.EditorActionProgress == 0 ? true : false); }

	private bool CheckGUIChange(int action_idx, bool start_state)
	{
	...
	private static void Copy()
	{
		...
		{
			var effect = Selection.activeTransform.GetComponent<EffectManager>();
	...
	private static void CopyHard()
	{
		if (Selection.activeTransform != null)
		{
			...
			if (effect != null)
			{
				EditorPrefs.SetString("EffectExport", effect.ExportData(true));
	...
	private void DrawEffectEditorPanel()
	{
		var main_editor_width = Instance.position.width - MainEditorX - WINDOW_BORDER_RIGHT;
		float HEIGHT_OF_DEFAULT_SETTINGS_GUI_PANEL = 215;
		var main_editor_height = Instance.position.height - WINDOW_BORDER_TOP - WINDOW_BORDER_BOTTOM - HEIGHT_OF_DEFAULT_SETTINGS_GUI_PANEL;

		var gui_y_offset = WINDOW_BORDER_TOP;
		...
		float x_offset = 0;
		var displayed_button = false;
		var continue_count = 0;
		if (font_manager.Playing)
		{
			LetterAnimation animation;
			for (var anim_idx = 0; anim_idx < font_manager.NumAnimations; anim_idx++)
			{
		...
		var text_area_height = Mathf.Clamp(Mathf.Clamp(font_manager.m_text.Split('\n').Length, 0, 6) * TEXT_AREA_LINE_HEIGHT, LINE_HEIGHT, 6 * TEXT_AREA_LINE_HEIGHT);
		...
		var animation_labels = new string[font_manager.NumAnimations];
		for (var anim_idx = 0; anim_idx < animation_labels.Length; anim_idx++)
		...
		if (selected_animation.m_letters_to_animate_option == LETTERS_TO_ANIMATE.CUSTOM)
		{
			...
			foreach (var character in font_manager.m_text)
			{
		...
		{
			gui_y_offset += LINE_HEIGHT;
			selected_animation.m_letters_to_animate_custom_idx = EditorGUI.IntField(new Rect(MainEditorX + 10, gui_y_offset, main_editor_width - 10, LINE_HEIGHT), new GUIContent((selected_animation.m_letters_to_animate_option == LETTERS_TO_ANIMATE.NTH_WORD ? "Word" : "Line") + " Number"), selected_animation.m_letters_to_animate_custom_idx);
		...
		LetterAction action;
		for (var idx = 0; idx < selected_animation.NumActions; idx++)
		{
			...
			if (action.FoldedInEditor)
			{
				...
				if (action.AudioEffectsEditorDisplay && action.NumAudioEffectSetups > 0)
				{
					...
					foreach (var a_effect in action.AudioEffectSetups)
						if (a_effect.m_editor_display)
							panel_height += LINE_HEIGHT * (a_effect.m_delay.NumEditorLines + a_effect.m_offset_time.NumEditorLines + a_effect.m_volume.NumEditorLines + a_effect.m_pitch.NumEditorLines + (a_effect.m_effect_assignment == PLAY_ITEM_ASSIGNMENT.CUSTOM ? 3 : 1) + 4);
						else
							panel_height += LINE_HEIGHT;
				}
				else
				...
				if (action.ParticleEffectsEditorDisplay && action.NumParticleEffectSetups > 0)
				{
					...
					foreach (var p_effect in action.ParticleEffectSetups)
						if (p_effect.m_editor_display)
							panel_height += LINE_HEIGHT * (p_effect.m_delay.NumEditorLines + p_effect.m_duration.NumEditorLines + p_effect.m_position_offset.NumEditorLines + p_effect.m_rotation_offset.NumEditorLines + (p_effect.m_effect_assignment == PLAY_ITEM_ASSIGNMENT.CUSTOM ? 2 : 0) + 8);
						else
							panel_height += LINE_HEIGHT;
				}
				else
		...
		for (var action_idx = 0; action_idx < selected_animation.NumActions; action_idx++)
		{
			...
			if (action.FoldedInEditor)
			{
				...
				if (action.AudioEffectsEditorDisplay)
				{
					...
					for (var idx = 0; idx < action.NumAudioEffectSetups; idx++)
					{
						...
						if (audio_setup.m_editor_display)
						{
							...
							if (audio_setup.m_effect_assignment == PLAY_ITEM_ASSIGNMENT.CUSTOM)
							{
								...
								if (audio_setup.m_effect_assignment_custom_letters == null)
									audio_setup.m_effect_assignment_custom_letters = new List<int>();
				...
				if (action.ParticleEffectsEditorDisplay)
				{
					...
					for (var idx = 0; idx < action.NumParticleEffectSetups; idx++)
					{
						...
						if (particle_setup.m_editor_display)
						{
							...
							if (particle_setup.m_effect_assignment == PLAY_ITEM_ASSIGNMENT.CUSTOM)
							{
								if (particle_setup.m_effect_assignment_custom_letters == null)
									particle_setup.m_effect_assignment_custom_letters = new List<int>();
							...
							EditorGUI.HelpBox(new Rect(ACTION_INDENT_LEVEL_1 + 340, gui_y_offset + LINE_HEIGHT, particle_setup.m_effect_type == PARTICLE_EFFECT_TYPE.LEGACY ? 150 : 230, LINE_HEIGHT), particle_setup.m_effect_type == PARTICLE_EFFECT_TYPE.LEGACY ? "zero == \"One Shot\"" : "zero == non-looping single playthrough", MessageType.Info);
	...
	private void DrawLoopTree(LetterAnimation selected_animation)
	{
		...
		var main_editor_height = Instance.position.height - WINDOW_BORDER_TOP - WINDOW_BORDER_BOTTOM;
		var num_actions = selected_animation.NumActions;
		var gui_y_offset = WINDOW_BORDER_TOP;
		var action_idx = 0;
		...
		LOOP_SCROLL_POS = GUI.BeginScrollView(new Rect(state_overview_x, gui_y_offset, MainEditorX - state_overview_x - 14, main_editor_height - (gui_y_offset - WINDOW_BORDER_TOP)), LOOP_SCROLL_POS, new Rect(0, 0, MainEditorX - 35, selected_animation.NumActions * ACTION_NODE_SPACING));
		...
		if (m_display_loops_tree)
		{
			...
			var num_loops = selected_animation.NumLoops;
			var my_style = EditorStyles.miniButton;
			...
			for (var loop_idx = 0; loop_idx < num_loops; loop_idx++)
			{
			...
			{
				var rect_idx = 0;
				foreach (var node_rect in m_state_node_rects)
				{
...
//							Debug.LogError("you joined : " + m_mouse_down_node_idx + " and " + rect_idx + " with button " + Event.current.button);
	...
	private static void Init()
	{
		...
		GetWindow(typeof(TextEffectsManager));
	}

	private void OnDisable() { EditorApplication.update -= UpdateManager; }

	private void OnEnable() { EditorApplication.update += UpdateManager; }

	private void OnGUI()
	{
		var edited_action = -1;
		var start_of_action = true;
	...
	private void OnInspectorUpdate() { Instance.Repaint(); }
