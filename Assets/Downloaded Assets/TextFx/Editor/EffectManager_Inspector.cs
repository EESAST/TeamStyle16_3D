...
public class EffectManager_Inspector : Editor
{
	...
	private bool m_old_baseline_override;
	private float m_old_char_size;
	private TextDisplayAxis m_old_display_axis;
	private float m_old_font_baseline;
	private float m_old_line_height;
	private float m_old_max_width;
	private Vector2 m_old_px_offset;
	private string m_old_text;
	private TextAlignment m_old_text_alignment;
	private TextAnchor m_old_text_anchor;
	...
	private int m_selected_animation_idx;
	private float m_set_text_delay;
	private float m_time_delta;

	private void OnDisable() { EditorApplication.update -= UpdateFunction; }
...
//		DrawDefaultInspector();
		...
		if (m_old_char_size != font_manager.m_character_size || m_old_display_axis != font_manager.m_display_axis || m_old_line_height != font_manager.m_line_height_factor || m_old_max_width != font_manager.m_max_width || !m_old_text.Equals(font_manager.m_text) || m_old_text_alignment != font_manager.m_text_alignment || m_old_text_anchor != font_manager.m_text_anchor || m_old_px_offset != font_manager.m_px_offset || m_old_baseline_override != font_manager.m_override_font_baseline || (font_manager.m_override_font_baseline && m_old_font_baseline != font_manager.m_font_baseline_override))
		...
		{
			var json_data = font_manager.ExportData();
		...
		{
			var json_data = font_manager.ExportData(true);
			EditorGUIUtility.systemCopyBuffer = json_data;
			EditorPrefs.SetString("EffectExport", json_data);
	...
	private void OnSceneGUI()
	{
		...
		var edit_detected = false;
		var edited_action = -1;
		var start_end_state = 0;
		var action_idx = 0;
		var position_offset = font_manager.transform.position;
		...
		for (var anim_idx = 0; anim_idx < font_manager.NumAnimations; anim_idx++)
		{
			...
			LetterAction action;
			for (var idx = 0; idx < letter_animation.NumActions; idx++)
			{
