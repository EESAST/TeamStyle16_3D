...
public class RuntimeDynamicSceneManager : MonoBehaviour
{
	...
	private static RuntimeDynamicSceneManager m_instance;
	public static Action m_reset_event; // Action event called to trigger all listening ShootTarget's to reset.
	private readonly Vector2 m_cursor_hot_spot = new Vector2(16, 16);
	...
	private int m_hit_effect_index;
	public EffectManager m_hit_effect_prefab; // Reference to text animation prefab being used on target hit.
	private EffectManager[] m_hit_effects; // Cached pool of text animation instances.
	...
	private void Start()
	{
		...
		for (var idx = 0; idx < EFFECT_POOL_SIZE; idx++)
	...
	public static void TargetHit(Vector3 position, TARGET_COLOUR colour)
	{
		...
		var hit_effect = m_instance.m_hit_effects[m_instance.m_hit_effect_index];
		...
		var audio_setup = hit_effect.GetAnimation(0).GetAction(0).GetAudioEffectSetup(0);
