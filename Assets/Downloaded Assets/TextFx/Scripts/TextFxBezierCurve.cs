...
public class TextFxBezierCurve
{
	...
	public void AddNewAnchor()
	{
		...
		{
			var last_point = m_anchor_points[m_anchor_points.Count - 1];
	...
	{
		var num_precision_intervals = NUM_CURVE_SAMPLE_SUBSECTIONS;
		Vector3? last_point = null;
		Vector3 current_point;
		float curve_length = 0;

		for (var idx = 0; idx < num_precision_intervals; idx++)
		{
	...
	public Vector3 GetCurvePoint(float progress, int num_anchors = 4, int curve_idx = -1)
	{
		...
		for (var idx = 1; idx < num_anchors; idx++)
			if (num_anchors == 4)
			{
				if (idx == 1)
	...
	public Vector3 GetCurvePointRotation(float progress, int curve_idx = -1)
	{
		...
		var point_dir_vec = GetCurvePoint(Mathf.Clamp(progress + 0.01f, 0, 1), curve_idx: curve_idx) - GetCurvePoint(Mathf.Clamp(progress - 0.01f, 0, 1), curve_idx: curve_idx);
	...
	public float[] GetLetterProgressions(ref LetterSetup[] letters, int letter_anchor)
	{
		...
		var progress_inc = 1f / NUM_CURVE_SAMPLE_SUBSECTIONS;
		float progress;
		var new_point = new Vector3();
		var last_point = new Vector3();
		...
		if (effect_manager_ref.m_text_alignment == TextAlignment.Center || effect_manager_ref.m_text_alignment == TextAlignment.Right)
			for (var curve_idx = 0; curve_idx < m_anchor_points.Count - 1; curve_idx++)
			{
		...
		for (var curve_idx = 0; curve_idx < m_anchor_points.Count - 1; curve_idx++)
			for (var idx = 0; idx <= NUM_CURVE_SAMPLE_SUBSECTIONS; idx++)
			{
		...
		for (var idx = letter_idx; idx < letters.Length; idx++)
	...
	public void OnSceneGUI(Vector3 position_offset, Vector3 scale)
	{
		...
		for (var p_idx = 1; p_idx < m_anchor_points.Count; p_idx++)
		{
			DrawCurvePoint(m_anchor_points[p_idx], position_offset, scale, p_idx == m_anchor_points.Count - 1, ref changed);

			Handles.DrawBezier(Vector3.Scale(m_anchor_points[p_idx - 1].m_anchor_point, scale) + position_offset, Vector3.Scale(m_anchor_points[p_idx].m_anchor_point, scale) + position_offset, (p_idx == 1 ? Vector3.Scale(m_anchor_points[p_idx - 1].m_handle_point, scale) : Vector3.Scale(m_anchor_points[p_idx - 1].m_anchor_point + (m_anchor_points[p_idx - 1].m_anchor_point - m_anchor_points[p_idx - 1].m_handle_point), scale)) + position_offset, Vector3.Scale(m_anchor_points[p_idx].m_handle_point, scale) + position_offset, Color.red, null, 2);
		}
	}
