...
{
	private Vector3 from_vec, to_vec;
	...
	public float m_text_anchoring_y_offset; // letter y-positional offset based on text anchoring settings. This value is not used when using a belzier curve
	...
	public Vector3 BottomLeft { get { return ((m_effect_manager_handle != null ? m_effect_manager_handle.Rotation : Quaternion.identity) * Vector3.Scale((mesh_verts != null ? mesh_verts[2] : Vector3.zero), (m_effect_manager_handle != null ? m_effect_manager_handle.Scale : Vector3.one))) + (m_effect_manager_handle != null ? m_effect_manager_handle.Position : Vector3.zero); } }
	public Vector3 BottomLeftLocal { get { return (mesh_verts != null ? mesh_verts[2] : Vector3.zero); } }
	public Vector3 BottomRight { get { return ((m_effect_manager_handle != null ? m_effect_manager_handle.Rotation : Quaternion.identity) * Vector3.Scale((mesh_verts != null ? mesh_verts[3] : Vector3.zero), (m_effect_manager_handle != null ? m_effect_manager_handle.Scale : Vector3.one))) + (m_effect_manager_handle != null ? m_effect_manager_handle.Position : Vector3.zero); } }
	public Vector3 BottomRightLocal { get { return (mesh_verts != null ? mesh_verts[3] : Vector3.zero); } }
	public Vector3 Center { get { return ((m_effect_manager_handle != null ? m_effect_manager_handle.Rotation : Quaternion.identity) * Vector3.Scale(CenterLocal, (m_effect_manager_handle != null ? m_effect_manager_handle.Scale : Vector3.one))) + (m_effect_manager_handle != null ? m_effect_manager_handle.Position : Vector3.zero); } }
	public Vector3 CenterLocal { get; private set; }
	public bool InReverse { get { return m_anim_state_vars.m_reverse; } }
	public Quaternion Rotation { get { return (m_effect_manager_handle != null ? m_effect_manager_handle.Rotation : Quaternion.identity) * RotationLocal; } }
	public Quaternion RotationLocal { get; private set; }
	public Vector3 Scale { get { return Vector3.Scale(ScaleLocal, (m_effect_manager_handle != null ? m_effect_manager_handle.Scale : Vector3.one)); } }
	public Vector3 ScaleLocal { get; private set; }
	public Vector3 TopLeft { get { return ((m_effect_manager_handle != null ? m_effect_manager_handle.Rotation : Quaternion.identity) * Vector3.Scale((mesh_verts != null ? mesh_verts[1] : Vector3.zero), (m_effect_manager_handle != null ? m_effect_manager_handle.Scale : Vector3.one))) + (m_effect_manager_handle != null ? m_effect_manager_handle.Position : Vector3.zero); } }
	public Vector3 TopLeftLocal { get { return (mesh_verts != null ? mesh_verts[1] : Vector3.zero); } }
	public Vector3 TopRight { get { return ((m_effect_manager_handle != null ? m_effect_manager_handle.Rotation : Quaternion.identity) * Vector3.Scale((mesh_verts != null ? mesh_verts[0] : Vector3.zero), (m_effect_manager_handle != null ? m_effect_manager_handle.Scale : Vector3.one))) + (m_effect_manager_handle != null ? m_effect_manager_handle.Position : Vector3.zero); } }
	...
	public void Init(EffectManager effect_manager)
	{
		...
		if (mesh_verts != null && mesh_verts.Length == 4)
			CenterLocal = (mesh_verts[0] + mesh_verts[1] + mesh_verts[2] + mesh_verts[3]) / 4;
	...
	public void SetAnimationVars(LetterSetup master_letter)
	{
		...
		m_anim_state_vars.m_active_loop_cycles = new List<ActionLoopCycle>();
		foreach (var loop_cycle in master_letter.AnimStateVars.m_active_loop_cycles)
	...
	{
		var text_data = text_datas[m_progression_variables.m_line_value];
	...
	private void SetNextActionIndex(LetterAnimation animation)
	{
		...
		for (var loop_idx = 0; loop_idx < m_anim_state_vars.m_active_loop_cycles.Count; loop_idx++)
		{
			current_loop = m_anim_state_vars.m_active_loop_cycles[loop_idx];

			if ((current_loop.m_loop_type == LOOP_TYPE.LOOP && m_anim_state_vars.m_action_index == current_loop.m_end_action_idx) || (current_loop.m_loop_type == LOOP_TYPE.LOOP_REVERSE && ((m_anim_state_vars.m_reverse && m_anim_state_vars.m_action_index == current_loop.m_start_action_idx) || (!m_anim_state_vars.m_reverse && m_anim_state_vars.m_action_index == current_loop.m_end_action_idx))))
			{
				// Reached end of loop cycle. Deduct one cycle from loop count.
				var end_of_loop_cycle = current_loop.m_loop_type == LOOP_TYPE.LOOP || m_anim_state_vars.m_reverse;
	...
	public void SetupLetterMesh(ref CustomCharacterInfo char_info)
	{
		...
		if (!m_flipped)
			// TR, TL, BL, BR
			m_base_vertices = new[] { new Vector3(m_width, (m_effect_manager_handle.FontBaseLine + m_y_offset), 0), new Vector3(0, (m_effect_manager_handle.FontBaseLine + m_y_offset), 0), new Vector3(0, m_height + m_effect_manager_handle.FontBaseLine + m_y_offset, 0), new Vector3(m_width, m_height + m_effect_manager_handle.FontBaseLine + m_y_offset, 0) };
		else
		// rotate order of vertices by one.
		// TL, BL, BR, TR
			m_base_vertices = new[] { new Vector3(0, (m_effect_manager_handle.FontBaseLine + m_y_offset), 0), new Vector3(0, m_height + m_effect_manager_handle.FontBaseLine + m_y_offset, 0), new Vector3(m_width, m_height + m_effect_manager_handle.FontBaseLine + m_y_offset, 0), new Vector3(m_width, (m_effect_manager_handle.FontBaseLine + m_y_offset), 0) };
	}
	...
	private void TriggerParticleEffects(AnimatePerOptions animate_per, PLAY_ITEM_EVENTS play_when)
	{
		if (m_current_letter_action.NumParticleEffectSetups > 0)
			foreach (var effect_setup in m_current_letter_action.ParticleEffectSetups)
				if (effect_setup.m_play_when == play_when && (effect_setup.m_effect_assignment == PLAY_ITEM_ASSIGNMENT.PER_LETTER || effect_setup.m_effect_assignment_custom_letters.Contains(m_progression_variables.m_letter_value)))
					if (!effect_setup.m_loop_play_once || m_anim_state_vars.m_active_loop_cycles == null || m_anim_state_vars.m_active_loop_cycles.Count == 0 || m_anim_state_vars.m_active_loop_cycles[0].FirstPass)
						m_effect_manager_handle.PlayParticleEffect(m_mesh, m_flipped, effect_setup, m_progression_variables, animate_per);
	}
	...
	private void UpdateLoopList(LetterAnimation animation)
	{
		...
		for (var idx = 0; idx < animation.NumLoops; idx++)
		{
			...
			if (loop.m_start_action_idx == m_anim_state_vars.m_action_index)
			{
				...
				var new_loop_cycle_span = loop.SpanWidth;

				var loop_idx = 0;
				foreach (var active_loop in m_anim_state_vars.m_active_loop_cycles)
				{
