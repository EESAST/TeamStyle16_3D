...
{
	private readonly Color _baseColor = new Color(1f, .423f, 0f, .5f);
	private readonly float _baseDuration = 3f;
	private readonly float _baseSize = 1f;
	private readonly Vector3 _baseVelocity = new Vector3(60f, 60f, 60f);
...
//	private float _baseDamping = 0.1300004f;
	...
	public void UpdateFireballA()
	{
		...
		var fadeWhite = new Color(1f, 1f, 1f, .5f);
		var fadeRed = new Color(.6f, .15f, .15f, .3f);
		var fadeBlue = new Color(.1f, .2f, .45f, 0f);
