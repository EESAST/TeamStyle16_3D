...
public class DetonatorCloudRing : DetonatorComponent
{
	...
	private readonly float _baseDuration = 5f;
	private readonly Vector3 _baseForce = new Vector3(0.162f, 2.56f, 0f);
	private readonly float _baseSize = 1f;
	private readonly Vector3 _baseVelocity = new Vector3(155f, 5f, 155f);
	...
	public void UpdateCloudRing()
	{
		...
		var color1 = Color.Lerp(color, (new Color(.2f, .2f, .2f, .6f)), 0.5f);
		var color2 = new Color(.2f, .2f, .2f, .5f);
		var color3 = new Color(.2f, .2f, .2f, .3f);
		var color4 = new Color(.2f, .2f, .2f, 0f);
