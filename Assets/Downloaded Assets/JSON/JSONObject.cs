...
public class JSONObject
{
				...
				else if (str[0] == '"')
				{
					...
					catch (FormatException)
					{
						...
						while (++offset < str.Length)
						{
							...
							{
								inProp = false;
								var inner = str.Substring(token_tmp, offset - token_tmp).Trim(WHITESPACE);
								if (inner.Length > 0)
								{
									if (type == Type.OBJECT)
										keys.Add(propName);
	...
	public JSONObject Copy() { return new JSONObject(print()); }
	...
	public string print(bool pretty = false) { return print(0, pretty); }
		...
		switch (type)
		{
			...
			case Type.OBJECT:
				...
				if (list.Count > 0)
				{
					...
					//BOP: This line shows up twice on purpose: once to remove the \n if readable is true and once to remove the comma
#endif
					str = str.Substring(0, str.Length - 1);
				}
#if(PRETTY)
