...
public class NoiseEffect : MonoBehaviour
{
	...
	private void OnRenderImage(RenderTexture source, RenderTexture destination)
	{
		...
		mat.SetVector("_GrainOffsetScale", new Vector4(Random.value, Random.value, Screen.width / (float)grainTexture.width * grainScale, Screen.height / (float)grainTexture.height * grainScale));
		mat.SetVector("_ScratchOffsetScale", new Vector4(scratchX + Random.value * scratchJitter, scratchY + Random.value * scratchJitter, Screen.width / (float)scratchTexture.width, Screen.height / (float)scratchTexture.height));
		mat.SetVector("_Intensity", new Vector4(Random.Range(grainIntensityMin, grainIntensityMax), Random.Range(scratchIntensityMin, scratchIntensityMax), 0, 0));
	...
	protected void Start()
	{
		...
		else if (!shaderRGB.isSupported) // disable effect if RGB shader is not supported
			enabled = false;
